...
import it.epicode.pizzas.entities.RestaurantTable;
...
public class BeansConfiguration {
	public static final String[] TOPPINGS = { "toppings_tomato", "toppings_cheese", "toppings_ham",
			"toppings_pineapple", "toppings_salami" };
	public static final String[] PIZZAS = { "pizza_margherita", "hawaiian_pizza", "salami_pizza", "salami_pizza_xl" };
	public static final String[] DRINKS = { "lemonade", "water", "wine" };
	public static final String[] TABLES = { "table1", "table2" };
	...
	Menu menuBean() {
		...
		var menu = new Menu();
		menu.getItems().addAll(pizzaList);
		menu.getItems().addAll(toppingsList);
		menu.getItems().addAll(drinkList);
		return menu;
	}

	@Bean(name = "table1")
	RestaurantTable table1() {
		return RestaurantTable.builder().withMaxPlaces(5).withName("Table n. 1").build();
	}

	@Bean(name = "table2")
	RestaurantTable table2() {
		return RestaurantTable.builder().withMaxPlaces(3).withName("Table n. 2").build();
	}
}
